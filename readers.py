    ...
    for row in cr:
        ...
        if numSamples == None:
            if useReplicates:
                ...
                for i in range(colsToIgnore, len(x)):
        ...
        try:
            ...
            if useReplicates:
                thisVectF = [float(np.mean([float(xx) for xx in ff.split(',')])) if ',' in ff else float(ff) for ff in
                             thisVect]
            else:
                thisVectF1 = [[float(xx) if xx != "" else 0 for xx in ff.split(',')] if ',' in ff else [float(ff)] for
                              ff in thisVect]
